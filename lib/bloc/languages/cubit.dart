...
class LanguagesCubit extends Cubit<LanguagesState> {
  ...
  String onBoardingDescription1() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      ...
      return 'له‌ڕێگای ئه‌م به‌رنامه‌یه‌وه‌ ده‌توانی به‌دوای هه‌موو شتێك دا بگه‌ڕێی به‌ ئاره‌زووی خۆت,وه‌هه‌روه‌ها بگه‌یت به‌ نزيكترين فرۆشیار له‌گه‌ڵ زانیاری فرۆشیار';
    }
    return '';
  }
  ...
  String onBoardingDescription2() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      ...
      return 'ئه‌م به‌رنامه‌یه‌ كات و ماندووبوونت بۆ ده‌گه‌ڕێنێته‌وه‌ له‌ هه‌ر كاڵاية ك‌ به‌دوای بگه‌ڕێیت به‌هۆی دانانی وێنه‌ یان وه‌سفكردنی ئه‌و شته‌ی داوای ده‌كه‌ی';
    }
    return '';
  }
  ...
  String searchForProduct() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      ...
      return 'به‌دوای كاڵاية ك‌ دا بگه‌ڕێ';
    }
    return '';
  }
  ...
  String streetName() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      ...
      return 'ناوی شه‌قام - یان نزیكترین شوێن لێی';
    }
    return '';
  }
  ...
  String enterNameOrShortDescriptionForTheItemYouAreLookingFor() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      ...
      return 'ناوێك یان وه سفێكی كورت بنوسه بۆ ئه و كاڵاية ك‌ به دوای ده گه ڕێی';
    }
    return '';
  }
  ...
  String ifYouWantToAddPictureOfTheItemYouAreLookingFor() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      ...
      return 'وێنه‌ی ئه‌و كاڵاية ك‌ به‌دوای دا ده‌گه‌رێیت';
    }
    return '';
  }
  ...
  String onlyFiveSearch() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      return 'لديك ١٠ عمليات بحث نشطه فقط، الرجاء حذف البحث بعد الوصول إلى المنتج المطلوب حتي تسطيع البحث مجدداً';
    } else if (SharedHelper.getCacheData(key: LANGUAGES) == 'EN') {
      ...
      return 'ته نیا 10 گه ڕانی كارات هه یه,تكایه له كاتی دۆزینه وه ی داواكارییه كه ت گه ڕانه كه ت بسڕه وه تاوه كو دووباره بتوانی گه ڕان بكه ی';
    }
    return '';
  }
  ...
  String searchingNow() {
    if (SharedHelper.getCacheData(key: LANGUAGES) == 'AR') {
      ...
      return 'گه‌ڕانه‌كه‌ ئێستا ته‌واو ده‌بێت له‌كاتی دۆزینه‌وه‌ی كاڵاية ك‌ له‌ یه‌كێك له‌ فرۆشگاكان,به‌شێك به‌ناوی په‌یوه‌ندی بكه‌ به‌رده‌ست ده‌بێت';
    }
    return '';
  }
